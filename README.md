...
在控制窗口选择「注视模式 · 使用摄像头」，点击开始并选择内置屏幕，然后允许摄像头访问。先正对屏幕约半秒完成就绪。模糊计时要求「没有面向屏幕」与「没有键盘鼠标操作」同时成立，任一条件中断即清零重新计时，所以正在用电脑时不会被模糊；可设置立刻或 3 秒至 5 分钟。玻璃每次从随机一侧缓慢扩散；默认重新面向屏幕即自动恢复清晰，勾选「保持模糊」或身份验证后才需要手动确认。
