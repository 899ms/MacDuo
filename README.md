...
离开时屏幕上会出现液态玻璃统计卡：本次连续注视时长、已离开时长与今日累计（仅保存在本机）。内置示例陪伴视频「豆姐」会在离开画面循环播放（静音、不阻止屏幕休眠），设置中可切换两种样式：「融入整屏玻璃」——氛围光晕铺满全屏、清晰主体居中、边缘融进玻璃，视频先完整播放一遍后统计卡再淡入；或「显示在卡片中」——视频作为圆角小窗嵌在卡片左侧。想换成自己的视频，替换 App 内 `Contents/Resources/AttentionCompanion.mp4` 即可（横屏素材会自动铺满屏宽）；删除该文件则关闭此功能。
