...
在控制窗口选择「注视模式 · 使用摄像头」（未开始时切换模式会自动弹出屏幕选择器）。先正对屏幕约半秒完成就绪。模糊计时要求「没有面向屏幕」与「没有键盘鼠标操作」同时成立，任一条件中断即清零重新计时，所以正在用电脑时不会被模糊；可设置立刻或 3 秒至 5 分钟。玻璃每次从随机一侧缓慢扩散成透亮的液态质感；默认重新面向屏幕即自动恢复清晰，勾选「保持模糊」或身份验证后才需要手动确认。

## 重点：注视统计 × 液态玻璃 × 陪伴视频（1.4.0）

<img src="docs/images/companion-preview.gif" width="220" align="right" alt="内置示例陪伴视频「豆姐」预览">

**你离开时，屏幕会告诉你刚刚专注了多久。**

- **注视统计**：只在「面向屏幕且未模糊」时累计，离开触发模糊即结束一段连续注视；液态玻璃统计卡显示本次连续时长（大号计时）、已离开时长与今日累计。数据仅保存在本机，午夜自动归零，菜单栏也会实时显示当前连续时长。
- **液态玻璃**：真透明玻璃卡（透出桌面、流动冷色、高光缓慢扫过），全屏毛玻璃同样是透亮的液态质感——高透过率、饱和度回补，不是一层灰雾。
- **陪伴视频**：内置示例视频「豆姐」在离开画面静音循环（不阻止屏幕休眠），两种样式随时切换：
  - **融入整屏玻璃**：模糊氛围光晕铺满全屏，清晰主体立于中央、边缘径向融进玻璃；视频先完整独舞一遍，统计卡再淡入到中下位置。
  - **显示在卡片中**：视频作为圆角玻璃小窗嵌在统计卡左侧。
- **换成你自己的**：替换 App 内 `Contents/Resources/AttentionCompanion.mp4` 即可（横屏素材自动铺满屏宽）；删除该文件则关闭视频功能。示例视频源文件见 [Resources/AttentionCompanion.mp4](Resources/AttentionCompanion.mp4)，Release 页也可单独下载。
